...
Introduction:
-------------
The main purpose of this assignment (#14) is to build a software architecture (Ros 1 in our case) and a simulation environment containing a conveyor belt where 3D boxes are spawned one after the other and differentiated depending on their colors. Accordingly a robot manipulator (Tiago) is able to estimate their color through an RGB-D camera, grasp them and put each box in its specific bin.
...
This image shows the simulation environment we created for this assignment, consisting of Tiago placed at end of a conveyor belt with colored bins to its sides where to put relative boxes:
<p>
