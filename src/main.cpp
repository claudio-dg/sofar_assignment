...
- Subscribes to:
-	
-	'/our_topic' : to receive feedback about a box reaching final position on the conveyor
-
-
- Client for Custom Services:
-
-	'/spawn_box_service' : to spawn a new box
-	'/box_tracker_service' : to make the conveyor start moving
-	'/color_detect_service' : to detect color of the box 
-
- Client for Action Services:
-
-	'/arm_controller/follow_joint_trajectory' : to move tiago's arm
-	'/torso_controller/follow_joint_trajectory' : to move tiago's torso
-	'/parallel_gripper_controller/follow_joint_trajectory' : to move tiago's gripper 
-
- Description:
-
- 	this node is the main one encharged of sinchronyzing Tiago's actions
-	contains function to create clients to tiago's arm/torso/grippers
-	and to create waypoints for the trajectory controllers.
-	The main() function consists of a while loop and calls movement functions and services to
-	1) spawn a random green/blue box
-	2) start the conveyor belt
-	3) recognise colors
-	4) move tiago to grab the box and release it in the correct box
- . */
...
// Generates a simple trajectory with three waypoints to move TIAGo's arm 
...
void waypoints_arm_postgraspGreen(control_msgs::FollowJointTrajectoryGoal& goal)
{
  ...
  // To be reached 5 seconds after starting along the trajectory
  goal.trajectory.points[index].time_from_start = ros::Duration(5.0);

 

}
// Generates a simple trajectory with three waypoints to move TIAGo's arm towards Blue bin
...
// Second trajectory point
  ...
  // To be reached 7 seconds after starting along the trajectory
...
// Generates a simple trajectory with one waypoint to move TIAGo's arm towards neutral position
...
// Generates a simple trajectory with one waypoint to move TIAGo's torso up
...
// Generates a simple trajectory with one waypoint to close TIAGo's gripper 
...
/*
-
- Callback for '/our_topic': to know that conveyor has stopped moving and the box has reached the grabbable position
-
- this function updates the global variable 'flag' in order to enter
- in the successive step of motion of the switch statement of the main
-
*/
...
//define the client for '/spawn_box_service' custom service
 SpawnerClient = nh.serviceClient<sofar_assignment::Spawner>("/spawn_box_service");

//define the client for '/box_tracker_service' custom service
 ConveyorClient = nh.serviceClient<sofar_assignment::Conveyor>("/box_tracker_service");

//define the subscriber to '/our_topic' topic
 ConveyorEndSub = nh.subscribe("our_topic",1,conveyorCallback);

//define the client for '/color_detect_service' custom service
